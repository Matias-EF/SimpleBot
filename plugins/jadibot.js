...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.connect().then(async ({user}) => {
      ...
      parent.reply(m.chat, '✅ *Conectado exitosamente con WhatsApp*\n*Nota: Esto es temporal*\n*Paragua-Bot se reinicia constantemente, tal que todos los sub bots tambien lo haran*\n\n' + JSON.stringify(user, null, 2), m)

      if (auth) return

      await parent.sendMessage(user.jid, `*Inicia sesión sin el codigo QR con el siguiente mensaje, envialo para reconectar con Paragua-Bot y/o diga conexion perdida...*\n\n*Puede obtener nuevamente este código QR con col comando #getcode, debe se seguir siendo sub bot para poder recibirlo*`, MessageType.extendedText)
  ...
  } else throw '*「 𝑷𝑨𝑹𝑨𝑮𝑼𝑨-𝑩𝑶𝑻 」*\n\n*❰ ⚠️ ❱ No se puede hacer un bot dentro de un sub bot!*\n*✳️ Use el comando #jadibot al numero oficial/principal de Paragua-Bot*\n\n*👉 https://wa.me/595986731337' + global.conn.user.jid.split`@`[0] + '?text=#jadibot*\n\n*「  𝑷𝑨𝑹𝑨𝑮𝑼𝑨-𝑩𝑶𝑻 」*'
